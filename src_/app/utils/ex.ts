...
export class OverlapService {
  ...
  loadZip(buffer: ArrayBuffer): void {
    // … your JSZip + parser.worker logic …
    // when you get a FeatureRec f:
  ...
  private testIntersect(a: Geometry, b: Geometry): Promise<boolean> {
    return new Promise(resolve => {
      const w = new Worker(
        new URL('./worker/intersect.worker', import.meta.url),
        { type: 'module' }
      );
      w.onmessage = ({ data }) => {
...
addEventListener('message', ({ data }) => {
  ...
  const { target, other } = data.data;
  const hit = booleanIntersects(
    { type: 'Feature', properties: {}, geometry: target },
    { type: 'Feature', properties: {}, geometry: other }
  );
